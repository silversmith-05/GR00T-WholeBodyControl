...
| --- | --- | --- | --- |
| 0 | 放开 | release（SDK 自定义预设） | `850 850 850 850 1000 θ` |
...
六路顺序：小指、无名指、中指、食指、拇指弯曲、拇指旋转。速度为六路 `200`，力阈值统一使用 `gear_sonic/utils/teleop/inspire_hand_controller.py` 中的 `HAND_FORCE = 500`。该参数同时应用于左右手的开合、旋转短按、旋转长按和独立测试工具，采集元信息的 `force` 也从它生成。需要调整时只修改这一处，随后重启遥操作和采集进程，并使用新数据集名称；现有数据集会校验力配置是否一致。这里的力阈值不是以 N·m 表示的力矩，SDK 内置预设校验中的 `200` 不随应用参数修改。

四指张开目标的唯一配置入口是 `gear_sonic/utils/teleop/inspire_hand_controller.py` 顶部的 `OPEN_FINGER_ANGLE = 850`，范围为整数 0..1000，左右手共用。遥操作、独立键盘工具、启动提示、反馈校验和采集元信息都从该值生成；以后调整四指张开程度只需改这一项。拇指弯曲仍为 1000，旋转按上述模式处理。

角度字段是设备标度，不是弧度或角度制。保留此前修正过的 `0=放开、1=闭合` 含义；当前预设修订 5 将四指张开目标统一配置，默认 850。更新后重启手部控制和记录进程，并使用新数据集名称，避免混用不同张开目标。SDK 0.3.0 自带预设未修改；应用使用其自定义预设和 `Command` 接口。普通开合/短按依次写力阈值、速度、角度。长按在本连接明确确认过配置的力阈值和速度后，只写变化的角度；每次写入均经过 SDK 保护检查。
...
仅做身体与因时手遥操作时，使用 [`start_inspire_teleop.sh`](inspire_teleop.md)；它默认在 NX 本机启动两个控制进程，不启动数据记录或 VLA。下面的入口用于带数据采集的遥操作。
...
`meta/info.json.script_config.hand` 记录型号、顺序、实际选用的闭合模板、旋转/长按规则、状态码和有效性；当前 `preset_revision=5`、`schema_version=4`。每帧另外保存实际配置，记录端不连接硬件。一个数据集的五指终点固定；不同终点不能续录到同一个数据集。`TypedLeRobotDataset` 支持本版自定义闭合模板，拒绝旧版两指/缺字段数据；本次不自动修改历史数据。

长按每个小步只在收到寄存器确认后推进 `action.thumb_rotation`；在途候选值记录于 `hand.write_thumb_rotation`，在途帧 `hand.training_valid=false`。这仍然是目标而非实测，实测只来自 `hand.angle_act`。普通开合/短按继续记录当前待执行目标。写入超时保留候选目标和未确认结果、使示范失效，不自动重试。长按在角度发送之前取消（可能只确认了配置的力阈值和速度）时，仅增加 `thumb_hold_cancel_count`，保留上一角度写入结果；不会把正常松键误记为运动失败。
